...
public class ReceivedLetterDAOImpl extends DAO implements ProjectDAO<Letter, String> {

	private static ProjectDAO<Letter, String> instance = new ReceivedLetterDAOImpl();

	private ReceivedLetterDAOImpl() {
	}
	...
	public List<Letter> selectAll() {
		...
		try {
			connect();
			String select = "SELECT * FROM received_letters";
	...
	public Letter selectOne(String timestamp) {
		...
		try {
			connect();
			String select = "SELECT * FROM received_letters WHERE timestamp = ?";
			pstmt = con.prepareStatement(select);
			...
			System.out.println("error from received letters selectOne()");
			e.printStackTrace();
		} finally {
			disconnect();
		}
	...
	public void insert(Letter letter) {
		try {
			connect();
			String insert = "INSERT INTO received_letters (sender_id, recipient_id, content, timestamp, is_read) VALUES (?,?,?,?,?)";
	...
	public void update(Letter letter) {
		try {
			connect();
			String update = "UPDATE received_letters SET is_read = ? WHERE timestamp = ?";
	...
	public void delete(Letter letter) {
		try {
			connect();
			String delete = "DELETE FROM received_letters WHERE timestamp = ?";
